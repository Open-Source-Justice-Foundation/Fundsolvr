...
import { DecodeResult, ProfilePointer } from "nostr-tools/lib/nip19";
      ...
      <div className="mx-auto flex h-screen w-full items-start justify-center text-sm text-gray-900 antialiased dark:text-white">
        <div className="mx-auto w-full max-w-3xl rounded-md border border-gray-600 p-4">
          <div className="mt-4 flex gap-x-4 px-4 py-3">
            <img className="h-32 w-32 cursor-pointer rounded-full" src={getProfile(relayUrl, publicKey)?.picture} />
            <div className="flex flex-col gap-y-4 px-4">
              <div className="flex items-center justify-between">
                <h2 className="text-3xl font-bold tracking-tight">{getProfile(relayUrl, publicKey)?.name}</h2>
              ...
              <span className="-mt-2 text-gray-500 dark:text-gray-400">{getProfile(relayUrl, publicKey)?.nip05}</span>

              <span className="text-lg">{getProfile(relayUrl, publicKey)?.about}</span>
            ...
            <div className="mx-8 my-8 flex justify-center gap-x-8 rounded-md border border-gray-600 px-4 py-4">
              ...
              <div className="flex items-center gap-x-2">
                ...
                <span className="text-lg text-gray-300/80">
                  {getProfile(relayUrl, publicKey)?.location ? getProfile(relayUrl, publicKey)?.location : "unknown"}
                </span>
              </div>
              ...
              <div className="flex items-center gap-x-1">
                ...
                <span className="text-lg text-gray-300/80">{getProfile(relayUrl, publicKey)?.lud16}</span>
              </div>
            </div>
