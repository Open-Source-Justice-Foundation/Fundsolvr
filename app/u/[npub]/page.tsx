    ...
    <div className="flex flex-col items-center justify-center px-4 pb-20 pt-10">
      <div className="flex w-full max-w-3xl flex-col gap-y-4">
        <div className="flex w-full items-center justify-between">
          <img className="h-28 w-28 cursor-pointer rounded-full ring-2 ring-white dark:ring-gray-300" src={getProfile(relayUrl, publicKey)?.picture} />
          <div className="flex gap-x-4">
            <Link
              className="flex items-center justify-center rounded-lg bg-gray-400 hover:bg-gray-500 dark:bg-gray-700/80 px-3 text-white dark:hover:bg-gray-700"
              href={`/messages/${nip19.npubEncode(getProfile(relayUrl, publicKey)?.publicKey || "")}`}
            >
              <PaperAirplaneIcon className="h-5 w-5" />
            </Link>

            <button className="flex items-center gap-x-2 rounded-lg bg-indigo-500 hover:bg-indigo-600 dark:bg-indigo-600 px-4 py-2 text-lg text-gray-100 dark:hover:bg-indigo-500">
        ...
        <span className="max-w-sm break-words text-lg text-gray-700 dark:text-gray-100 md:max-w-xl">{getProfile(relayUrl, publicKey)?.about}</span>
