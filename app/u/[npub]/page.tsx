...
import { getITagValues, verifyGithub } from "@/app/lib/utils";
  ...
  async function setupProfile(event: any) {
    ...
    for (let tag of iTags) {
      if (tag[0] === "github") {
        ...
        break; // Assuming you only need one GitHub tag, exit loop once found
      }
    }

    const profile: Profile = {
      relay: relayUrl, // Note: relayUrl isn't defined in your provided code
    ...
    setProfile(profile); // Note: setProfile isn't defined in your provided code
  }
    ...
    <div className="flex flex-col items-center justify-center px-4 pb-20 pt-10">
      <div className="flex w-full max-w-3xl flex-col gap-y-4">
        ...
        {getProfile(relayUrl, publicKey)?.github && (
          <a target="_blank" rel="nofollow noopener noreferrer" href={`https://github.com/${getProfile(relayUrl, publicKey)?.github}`}>
            <svg className="h-8 w-8 fill-gray-400" width="24" height="24" viewBox="0 0 24 24">
